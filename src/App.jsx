    ...
    >
      ...
      <main className="relative z-10 flex-1 flex items-center justify-center px-6 py-20">
        <div className="w-full max-w-xl mx-auto flex flex-col items-center text-center">
          {/* photo */}
          <div className="cs-elem opacity-0 w-36 md:w-44 mb-8">
            <img
              src={tristanPhoto}
          ...
          {/* call to action */}
          <div className="cs-elem opacity-0 mt-10 flex flex-col sm:flex-row items-center gap-4">
            <a
              href={`mailto:${EMAIL}`}
              className="inline-flex items-center justify-center gap-2 bg-clay text-cream font-heading font-semibold px-7 py-3.5 rounded-full text-sm md:text-base transition-transform hover:-translate-y-0.5 hover:shadow-lg hover:shadow-clay/25"
            >
              Mail mij direct
            </a>
            <a
              ...
              className="inline-flex items-center justify-center gap-2 border border-cream/25 text-cream font-heading font-semibold px-7 py-3.5 rounded-full text-sm md:text-base transition-colors hover:border-clay hover:text-clay"
            >
      ...
      <footer className="relative z-10 px-6 py-8 border-t border-cream/10">
        <div className="max-w-xl mx-auto flex flex-col sm:flex-row items-center justify-center gap-x-6 gap-y-2 text-center font-mono-brand text-xs md:text-sm text-cream/45">
          ...
          <a href={`mailto:${EMAIL}`} className="hover:text-clay transition-colors">
            {EMAIL}
          </a>
          <a href={GSM_HREF} className="hover:text-clay transition-colors">
            {GSM}
          </a>
        </div>
      </footer>
